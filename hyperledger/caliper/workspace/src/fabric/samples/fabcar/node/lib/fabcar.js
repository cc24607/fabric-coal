...
class FabCar extends Contract {
    ...
    async queryCar(ctx, textNumber) {
        const carAsBytes = await ctx.stub.getState(textNumber); // get the car from chaincode state
        if (!carAsBytes || carAsBytes.length === 0) {
            throw new Error(`${textNumber} does not exist`);
        }
        console.log(carAsBytes.toString());
        return carAsBytes.toString();
    }

    async createCar(ctx, textNumber, make, model, color, owner) {
        ...
        await ctx.stub.putState(textNumber, Buffer.from(JSON.stringify(car)));
    ...
    async deleteCars(ctx, cars) {
        ...
        for (let textNumber of deleteCars) {
            console.log(`Deleting car ${textNumber} from world state`)
            await ctx.stub.deleteState(textNumber);
        }
    }
    ...
    async changeCarOwner(ctx, textNumber, newOwner) {
